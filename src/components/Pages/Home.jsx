...
import { getAllData, subCategories } from "../../features/dataThunks";
...
const Home = () => {
  ...
  useEffect(() => {
    if (!data || data.length === 0) dispatch(getAllData());
  }, [dispatch, data]);
  ...
  const handleMoreClick = (category_id) => {
    dispatch(subCategories(category_id));
  };
  return (
    <>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {data.map((item, index) => (
              <div
                key={index}
                className="bg-white shadow-lg rounded-lg overflow-hidden p-6  hover:bg-gray-100 transition-all"
              >
                ...
                {item.products && item.products.length > 0 ? (
                  ...
                  <div className="bg-white shadow-md rounded-lg p-4 mb-4 hover:shadow-2xl hover:bg-gray-50 transition-all">
                    <center>
                      <img src={giff} alt="No Products" className="w-16 h-16" />
                    </center>
                  </div>
                )}
                <NavLink
                  to={"/user-dashboard"}
                  onClick={() => handleMoreClick(item?.category_id)}
